...
# BKC nightly: a nightly version plus a `bkc`
# local segment carrying the bkc date. The three producer pipelines each spell
# the separator differently; classify canonicalizes them all to the wheel form
# `<nightly-version>+bkc.<bkc-date>`. This regex stays tolerant of all three so it
# also matches a raw producer string defensively:

...
isolation against their own assumption of the envelope shape. That let the
lifecycle key drift (`dispatch_kind` vs `event_type`) without any test
failing.
...
NOTIFY_QUARTZ_DIR = Path(__file__).resolve().parents[2] / "notify_quartz"
if str(NOTIFY_QUARTZ_DIR) not in sys.path:
    sys.path.insert(0, str(NOTIFY_QUARTZ_DIR))
